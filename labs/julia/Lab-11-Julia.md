...
1. Define an **adjacency matrix** for a graph with four vertices and five edges. Use a matrix (2D array) to represent it. Example:
...
2. Write a function `matmul(A, B)` to multiply two matrices. Ensure it works for compatible matrix dimensions. Test with small examples.
   
   Note: Julia has built-in matrix multiplication with `*`, but implement your own for this exercise.
...
4. Write a function `transpose_manual(M)` that computes the **transpose** of a matrix $M$, returning a new matrix.
   
   Note: Julia has `transpose()` and the `'` operator, but implement your own.
...
6. Write a function `matrix_power(M, k)` that computes the $k$-th power of an adjacency matrix using repeated multiplication.
   ...
   Note: Julia has `M^k` built-in, but implement your own.

7. Use your function to compute $A^3$ from problem #1. Interpret the value at position `[2,4]` as the number of walks of length 3 from vertex 2 to vertex 4.
   
   Note: Julia uses 1-based indexing, so `[2,4]` refers to row 2, column 4.
...
Last modified Sun, January 30, 2026 09:20:23 PM EDT
</i>
