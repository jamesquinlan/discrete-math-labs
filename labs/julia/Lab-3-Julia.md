...
In this lab, you will explore conjectures and code to solve mathematical problems such as finding the smallest prime factor of a given number or determining the digital sum of a positive integer. You are encouraged to reuse and adapt functions developed in previous labs, such as `is_prime` and `factorial`, as well as array-processing tools, to solve these problems efficiently.
...
3. Write a function that finds the "next prime" given any integer. Write another function that finds the previous prime number. For example, `nextprime(17)` returns 19 and `prevprime(17)` returns 13. Note: Julia has built-in `nextprime()` and `prevprime()` functions, but implement your own for practice using your `isprime` function from the previous lab.
...
$$
This only exists when $\gcd(a, m) = 1$. Write a function `modinv(a, m)` that returns the modular inverse of $a$ modulo $m$, or indicates that no inverse exists (return `nothing`). Use the **extended Euclidean algorithm** to compute this.
...
9. The digital sum is a number between $0$ and $9$ obtained by repeatedly summing the digits of a number. For example, the digital sum of $591 \rightarrow 5 + 9 + 1 = 15 \rightarrow 1 + 5 = 6$. That is, the digital sum of $591$ is $6$. Write a digital sum function, $d(n)$, that takes a positive integer and returns the digital sum. Then, find the digital sum of $519, 18974$, and $100!$. *Hint*: Convert the number to a string using `string()` to access individual digits.
